...
import type { ShardRowCount, StorageNode, TableShard, VirtualIndexEntry } from '../../src/engine/topology/types';
...
describe('Topology Durable Object', () => {
	...
	describe('updateTopology', () => {
		...
		it('should add tables and create table_shards', async () => {
			...
			const userShards = topology.table_shards.filter((s: TableShard) => s.table_name === 'users');
	...
	describe('Virtual Indexes', () => {
		...
		it('should batch upsert multiple index entries efficiently', async () => {
			...
			const alice = topology.virtual_index_entries.find((e: VirtualIndexEntry) => e.key_value === 'alice@example.com');
			const bob = topology.virtual_index_entries.find((e: VirtualIndexEntry) => e.key_value === 'bob@example.com');
			const charlie = topology.virtual_index_entries.find((e: VirtualIndexEntry) => e.key_value === 'charlie@example.com');
		...
		it('should handle large batch upsert', async () => {
			...
			const user0 = topology.virtual_index_entries.find((e: VirtualIndexEntry) => e.key_value === 'user0@example.com');
			const user50 = topology.virtual_index_entries.find((e: VirtualIndexEntry) => e.key_value === 'user50@example.com');
			const user99 = topology.virtual_index_entries.find((e: VirtualIndexEntry) => e.key_value === 'user99@example.com');
	...
	describe('getQueryPlanData', () => {
		describe('SELECT queries', () => {
			...
			it('should use composite index for AND queries', async () => {
				...
				expect(planData.shardsToQuery.map((s: ShardInfo) => s.shard_id).sort()).toEqual([0, 2]);
			});
		});
		...
		describe('DELETE queries', () => {
			...
			it('should support IN queries with virtual index', async () => {
				...
				const shardIds = planData.shardsToQuery.map((s: ShardInfo) => s.shard_id).sort();
		...
		describe('Error handling', () => {
			it('should throw error if table does not exist', async () => {
				...
				await expect(stub.getQueryPlanData('nonexistent', statement, [123])).rejects.toThrow("Table 'nonexistent' not found in topology");
			});
