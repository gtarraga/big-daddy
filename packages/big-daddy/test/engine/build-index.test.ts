...
describe('Build Index', () => {
	it('should build index on table with 10 rows via public interfaces', async () => {
		...
		const user1Entry = topology.virtual_index_entries.find((e) => e.key_value === 'user1@example.com');
		const user5Entry = topology.virtual_index_entries.find((e) => e.key_value === 'user5@example.com');
		const user10Entry = topology.virtual_index_entries.find((e) => e.key_value === 'user10@example.com');
	...
	it('should manually trigger build index job and verify topology update', async () => {
		...
		const topologyStub = env.TOPOLOGY.get(env.TOPOLOGY.idFromName(dbId));
		await topologyStub.createVirtualIndex('idx_name', 'users', ['name'], 'hash');
		...
		const names = topology.virtual_index_entries.map((e) => e.key_value);
	...
	it('should build composite index on multiple columns', async () => {
		...
		const topologyStub = env.TOPOLOGY.get(env.TOPOLOGY.idFromName(dbId));
		await topologyStub.createVirtualIndex('idx_category_subcategory', 'products', ['category', 'subcategory'], 'hash');
	...
	it('should handle NULL values correctly when building index', async () => {
		...
		const nullEntry = topology.virtual_index_entries.find((e) => e.key_value === 'null');
	...
	it('should build index across multiple shards correctly', async () => {
		...
		const pendingEntry = topology.virtual_index_entries.find((e) => e.key_value === 'pending');
		const completedEntry = topology.virtual_index_entries.find((e) => e.key_value === 'completed');
	...
	it('should build index on table with duplicate values', async () => {
		...
		const topologyStub = env.TOPOLOGY.get(env.TOPOLOGY.idFromName(dbId));
		await topologyStub.createVirtualIndex('idx_level', 'logs', ['level'], 'hash');
		...
		const levels = topology.virtual_index_entries.map((e) => e.key_value).sort();
		...
		const infoEntry = topology.virtual_index_entries.find((e) => e.key_value === 'INFO');
